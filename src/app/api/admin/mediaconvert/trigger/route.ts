...
export async function POST(request: NextRequest) {
  try {
    ...
    const { inputKey, filename } = body

    if (!inputKey || !filename) {
      return NextResponse.json(
        { error: 'Missing required fields: inputKey, filename' },
        { status: 400 }
      )
    }

    const baseName = filename.replace(/\.[^/.]+$/, '')
    const outputDir = inputKey.substring(0, inputKey.lastIndexOf('/') + 1) + 'processed/'
        ...
        OutputGroups: [
          {
            ...
            OutputGroupSettings: {
              ...
              FileGroupSettings: {
                Destination: `s3://${BUCKET_NAME}/${outputDir}`,
              },
            },
            Outputs: [{
              NameModifier: '-thumb',
    ...
    return NextResponse.json({
      ...
      inputKey,
      outputPath: `s3://${BUCKET_NAME}/${outputDir}`,
      expectedOutputs: {
        thumbnail: `${baseName}-thumb.0000000.jpg`,
